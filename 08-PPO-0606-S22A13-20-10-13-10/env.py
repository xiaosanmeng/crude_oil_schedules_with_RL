...
class Env():
    ...
    def charge(self,distiller,tank,rate):
        ...
        if (rate != 0):
            ...
            # while (sub_schedule_index < len(self.RT[distiller - 1]) and self.RT[distiller - 1][
            #     sub_schedule_index] == 0):
            #     sub_schedule_index += 2
    ...
    def selectDistiller(self,type):
        ...
        # 》》》》》》》》》》》》》》》》》》》》3管道混合成本最小》》》》》》》》》》》》》》》》》》》》》》》》》》》》》》
        if type == 3:
            if len(self.schedule_pipe)==0:#（首次调度的情况默认选择蒸馏塔1)
                distiller=1
            else:
                residual_crude_oil_type_pipe=self.schedule_pipe[-1][1]
                min_mix_pipe_cost = float('inf')
                for RTi in self.RT:
                    if(RTi[2]==0 and RTi[4]==0):
                        continue
