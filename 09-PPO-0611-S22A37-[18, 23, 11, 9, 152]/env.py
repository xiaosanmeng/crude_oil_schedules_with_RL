...
#优化refine，charge ,selectDistiller函数(type==3存在bug，优化self_init 的action,state和self.target
...
class Env():
    ...
    def __init__(self):
        ...
        self.reward_weight = [1, 1, 1, 1, 0.3]
    ...
    def render(self,renderTime):
        ...
        for i in range(len(data)):
            for j in range(len(data[i])):
                plt.barh(y=4-i, width=(data[i][j][4]-data[i][j][3]),height=0.5, left=data[i][j][3],edgecolor='k', color=color[data[i][j][1]],hatch=(hatch[data[i][j][5]] if len(data[i][j])==6 else ''))
