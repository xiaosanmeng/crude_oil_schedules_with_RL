...
def evaluate_policy(env, agent, turns = 3,show=False):
	total_scores = 0
	for j in range(turns):
		...
		while not done:
			...
			a = agent.select_action(s, deterministic=True)#此时传入的a为数组下标
			actions.append(a)
			...
			s = s_next
		if show:
			print("actions:",actions,"obj:",env.preTarget)
	return int(total_scores/turns),env.preTarget,actions
