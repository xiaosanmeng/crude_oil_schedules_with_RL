...
class Env():
    ...
    def __init__(self):
        ...
        #泵速：[0 833,1250,1375,dynamic]
        #rateListIndex=math.ceil(action/12)
        #   rateListIndex=0:0
        #   rateListIndex=1:833
        #   rateListIndex=2:1250
        #   rateListIndex=3:1375
        #   rateListIndex=4:dynamic
        ...
        self.rateList=[0,833,1250,1375,"dynamic"]#1+4
    ...
    def step(self, action,render=False,rendertime=0.25):
        ...
        TankAndDistiller=self.TDList[action % 12]#[11,12,13,14,  21,22,23,24,  31,32,33,34]
        rateType=self.rateList[math.ceil(action/12)]#[0,833,1250,1375,"dynamic"]

        disitller =self.selectDistiller( TankAndDistiller // 10)
        tank=self.selectTank(TankAndDistiller%10,disitller)

        #执行调度↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓ ↓
        self.charge(disitller,tank,rateType)
        self.refine()
        self.isSecureState()
    ...
    def render(self,renderTime):
        ...
        for i in range(len(data)):
            for j in range(len(data[i])):
                ...
                plt.text(y=4.35-i,x=(data[i][j][4]+data[i][j][3])/2,s=("v"+str(data[i][j][-1]) if i==3 else ""),ha='center', va='center', color='k',fontsize=10)
        #其他设置
    ...
    def charge(self,distiller,tank,rateType):
        ...
        if (rateType != 0):
            ...
            if (endTime < startTime):
                endTime = startTime
            #schedule
            schdule = [0, 0, 0, startTime, endTime, rateType]
            self.schedule_pipe.append(schdule)
            # 更新time_ODT
    ...
    def selectRate(self,type,volume,distiller,startTime):
        ...
        if type=="dynamic":
            ...
            if volume/(endTime-startTime)>1375:
                ...
                return 1375
            #安全状态：
            rate=volume/(endTime-startTime) if volume/(endTime-startTime) >= 833.3 else 833.3
