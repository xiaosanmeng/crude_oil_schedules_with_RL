    ...
    def render(self,renderTime):
        ...
        for i in range(len(data)):
            for j in range(len(data[i])):
                plt.barh(y=4-i, width=(data[i][j][4]-data[i][j][3]),height=0.5, left=data[i][j][3], color=color[data[i][j][1]],edgecolor='k',hatch=(hatch[data[i][j][5]] if len(data[i][j])==6 else ''))
