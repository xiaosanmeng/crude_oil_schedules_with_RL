    ...
    def step(self, action,render=False,rendertime=1):
        ...
        if(self.SecureState):
            ...
            for log_tanki in self.log_tank:
                if(len(log_tanki)>2):
                    currUsedTank+=1
            reward_used_tank-=100


        reward_used_tank+=(self.usedTank-currUsedTank)
        self.usedTank=currUsedTank

        # ++++++++++++++++++++++++++++++++reward_mix_pipe++++++++++++++++++++++++++++++++++++++++++
        self.isSecureState()
        reward_mix_pipe = 0
        if (self.SecureState):
            curCOT=self.schedule_pipe[-1][1]
            if(curCOT!=0):#非停运
                if(self.preCOT!=None):

                    reward_mix_pipe = -self.Mp[self.preCOT - 1][curCOT - 1]
                self.preCOT=curCOT
        else:
            reward_mix_pipe=-100
        ...
        if(self.SecureState):
            ...
            for log_tank_i in self.log_tank:
                for i in range(len(log_tank_i)-3):
                    curMixTank+=self.Mt[log_tank_i[2+i]-1][log_tank_i[3+i]-1]
            reward_mix_tank=self.mixTank-curMixTank
            self.mixTank=curMixTank
        else:
            reward_mix_tank=-100
        ...
        if (self.SecureState):
            ...
            for schedule_distiller_i in self.schedule_distiller:
                curChangeTank+=len(schedule_distiller_i)
            reward_change_tank=self.changeTank-curChangeTank
            self.changeTank=curChangeTank
        else:
            reward_change_tank = -100
        ...
        #         state[6+i]=0
        #     else:
        #         required_type=self.RT[i][1] if self.RT[i][2]!=0 else self.RT[i][3]
        #         for tank in self.TK:
        #             if(tank[3]==0 and self.log_tank[tank[0]-1][1]<=self.time_ODT and self.log_tank[tank[0]-1][-1]==required_type):
        #                 state[6+i]+=1
        ...
        # ++++++++++++++++++++++++++++++++done++++++++++++++++++++++++++++++++++++++++++
        done = ((self.RT[0][2]+self.RT[0][4] + self.RT[1][2]+self.RT[1][4] + self.RT[2][2]+self.RT[2][4])== 0 or not self.SecureState)
        ...
        return state,reward_change_tank,done
                ...
                for tank in self.TK:
                    ...
                    if (distiller[2] > 0 and tank[1] == distiller[1] and tank[3] != 0 and self.log_tank[tank[0] - 1][
                        ...
                        if (len(self.schedule_distiller[distiller[0] - 1]) != 0):
                            ...
                            startTime = 0
                        endTime = startTime + (refineVolume / self.F_SDU[distiller[0] - 1])
                        ...
                        # ---------------------------------------------------------------
                        self.log_tank[tank[0] - 1].append(oilType)
    ...
    def charge(self,distiller,tank,rate):
        ...
        if (rate != 0):
            ...
            if (volume != 0):
                ...
                # 更新time_ODT
                self.time_ODT = startTime + volume / rate
                # 充油
                self.TK[tank - 1][1] = cot
                self.TK[tank - 1][3] = volume
                self.log_tank[tank - 1][0] = startTime + volume / rate + self.RESIDENCE_TIME
