...
class Env():
    ...
    def __init__(self):
        ...
        self.INITSTATE = self.updataState()
        ...
        if (self.SecureState):
            ...
            for log_tanki in self.log_tank:
                if (len(log_tanki) > 2):
                    self.target[3]+= 1
            #++++++++++++++++++++++++++++++++++++REWARD
            reward = 1.0 * (self.preTarget[0] - self.target[0]) + 1.0 * (self.preTarget[1] - self.target[1]) + 1.0 * (
                        self.preTarget[2] - self.target[2]) + 1.0 * (self.preTarget[3] - self.target[3])
    ...
    def selectTank(self,type,distiller):
        ...
        if type==4:
            #预接收原油的类型
            received_crude_oil_type=self.RT[distiller-1][1] if self.RT[distiller-1][2]!=0 else self.RT[distiller-1][3]
            min_mix_tank_cost=float('inf')

            for emptyTank in emptyTK:

                if(len(self.log_tank[emptyTank[0]-1])>2):
                    residual_crude_oil_type=self.log_tank[emptyTank[0]-1][-1][0]
                    mix_tank_cost=self.Mt[residual_crude_oil_type-1][received_crude_oil_type-1]
                else:
                    mix_tank_cost=0
